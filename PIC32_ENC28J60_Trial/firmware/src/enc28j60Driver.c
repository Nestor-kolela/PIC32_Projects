...
void ENC28J60_Init(void)
{
	...
	do
	{
		tempValue = Read_Control_Register(ESTAT, false); //read ESTAT register
	...
	Bank_Select(2);
	Write_Control_Reg(MACON1, 2, 0x05);
#ifdef FULL_DUPLEX
	Write_Control_Reg(MACON3, 2, 0x23);
#else
	Write_Control_Reg(MACON3, 2, 0x22);
#endif
...
#if LOOP_BACK
	write_PHY_Register(PHCON1, 0x4000); // half duplex... 
#endif
    
	write_PHY_Register(PHCON2, 0x0100);

	write_PHY_Register(PHLCON, 0x3422);// 472
...
bool ETH_CheckLinkUp(void)
{
    ...
    phstat2 = read_PHY_Register(PHSTAT2);// ENC28_PhyRead(J60_PHSTAT2);
...
void Bank_Select(uint8_t bank)
{
	...
	{
		tempValue = Read_Control_Register(ECON1, false);
		tempValue &= 0xFC;
...
void SPI_send(uint8_t data)
{
    ...
    while(SPI2_IsTransmitterBusy() == true)
    {
        continue; 
    }
...
uint16_t read_PHY_Register(uint8_t reg)
{
	...
	do
	{
		status = Read_Control_Register(MISTAT, true);
	...
	returnValue = Read_Control_Register(MIRDL, true);
	uint8_t u8HighByte = Read_Control_Register(MIRDH, true);
...
void enc28j60Write_Buffer(uint16_t len, uint8_t * data)
{
	...
	SPI_send(WRITE_BUFFER_MEM | 0x1A); //start by sending this command WBM
    SPI2_WriteRead(data, len, NULL, 0); 

	enc28j60_CS_High(); //make it high
}

uint8_t etherNetBuffer[2048]; //get from the stack quickly
...
bool enc28j60_BuildAndSend(uint8_t * buf, uint16_t length)
{
	...
	*(etherNetBuffer + index++) = 0x0E;

	//Load 6 bytes for the destination address.
	for(uint8_t cnt = 0; cnt < 6; cnt++)
	{
		*(etherNetBuffer + index++) = destAddress[cnt];
	}

	//Load 6 bytes for the source address.
	for(uint8_t cnt = 0; cnt < 6; cnt++)
	{
		*(etherNetBuffer + index++) = macAddress[cnt];
	}

	//Load the length
	*(etherNetBuffer + index++) = (length & 0xFF);
	*(etherNetBuffer + index++) = (length >> 8);

	uint16_t strLength = strlen((char * ) buf);

	for(uint8_t cnt = 0; cnt < strLength; cnt++)
	{
		*(etherNetBuffer + index++) = *(buf + cnt);
	}

	//All data has been load so we can proceed...

	enc28j60Write_Buffer(index, etherNetBuffer);
...
{
	Bank_Select(0);
	uint8_t returnValue = Read_Control_Register(ECON1, false);
